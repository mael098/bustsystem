...
import type { Student, Route, Notification, Location, StudentStatus, RouteStatus } from '@/types';
import { mockStudents, mockRoute, mockNotifications, simulatedRoutePath } from '@/data/mock-data';
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  // Student functions
  const addStudent = useCallback((studentData: Omit<Student, 'id' | 'status'>) => {
  ...
  const getStudentsByParent = useCallback((parentId: string) => {
    return students.filter(s => s.parentId === parentId);
  }, [students]);

  // Route functions
  const startRoute = useCallback((type: 'morning' | 'afternoon') => {
    const routeStatus: RouteStatus = type === 'morning' ? 'morning_pickup' : 'afternoon_pickup';
    ...
    const newRoute: Route = {
      ...
      points: students.map((student, index) => ({
        ...
        location: type === 'morning' ? student.homeLocation : student.school.location,
        address: type === 'morning' ? student.homeAddress : student.school.address,
        completed: false,
      })),
      startTime: new Date(),
    };
  ...
  const completeRoutePoint = useCallback((pointId: string) => {
    ...
    setCurrentRoute(prev => {
      ...
      return {
        ...prev,
        points: prev.points.map(p => p.id === pointId ? { ...p, completed: true } : p),
      };
    });
    ...
    if (point) {
      const newStatus: StudentStatus = 
        currentRoute.status === 'morning_pickup' ? 'picked_up' :
        currentRoute.status === 'to_school' ? 'at_school' :
        currentRoute.status === 'afternoon_pickup' ? 'returning' : 'delivered';
  ...
  const endRoute = useCallback(() => {
    ...
    setCurrentRoute(prev => prev ? { ...prev, status: 'completed', endTime: new Date() } : null);
    
    // Mark all students as delivered/at school based on route type
    const finalStatus: StudentStatus = 
      currentRoute.status === 'morning_pickup' || currentRoute.status === 'to_school' 
        ? 'at_school' 
        : 'delivered';
  ...
  const addNotification = useCallback((notifData: Omit<Notification, 'id' | 'timestamp' | 'read'>) => {
